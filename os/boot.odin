...
boot :: proc(k: ^Kernel) {
    ...
    // Two processes from one image, passing a frame of pixels back and forth. A
    // message carries 64 bytes; the frame is 16K, so it travels as a grant -- the
    // memory moves, and nothing copies it.
